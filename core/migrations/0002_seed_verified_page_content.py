...
def seed_verified_page_content(apps, schema_editor):
    ...
    AboutPageContent.objects.get_or_create(
        singleton_key=1,
        defaults={
            "role": "计算机科学与技术本科生 · 后端开发",
            "direction": "聚焦 Python 自动化、Django 后端与多 Agent 工作流",
...
def remove_seeded_page_content(apps, schema_editor):
    apps.get_model("core", "HomePageContent").objects.filter(
        singleton_key=1
    ).delete()
    apps.get_model("core", "AboutPageContent").objects.filter(
        singleton_key=1
    ).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            seed_verified_page_content,
            remove_seeded_page_content,
        ),
    ]
